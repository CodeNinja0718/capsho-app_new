...
export function getRandomCaptionTemplate (context: DescriptionContext) {
  const {
    hook,
    title,
    hookStatement,
  ...
  const getHookOrQuote = (isGuest: boolean): string => {
    const templates: Prompt = {
      ...
      guest: [
        ...
        () => `${hook}\n\nIn this week's episode with ${guestName} I cover:\n${findings}\n\n${title} is now live. Listen at the link in bio / comments`,
  ...
  const strategyCaptionTemplates: Template = {
    ...
    guest: {
      ...
      boxerN1: [
        ...
        () => `${cht.value}\n\nIn this week's episode featuring ${guestName}, I cover:\n${findings}\n\nListen at the link in bio / comments`
      ],
      boxerN2: [
      ...
      marvel: [
        ...
        () => `${cht.value}\n\nIn this episode featuring ${guestName}, I also cover:\n${findings}\n\nListen at the link in bio / comments`
      ],
      jawDropper: [
        () => `${cht.value}\n\n${title} with ${guestName} is now live. Listen at the link in bio / comments`,
        () => `${cht.value}\n\nIn this episode with ${guestName} I also cover:\n${findings}\n\nListen at the link in bio / comments`
      ],
      hookOrQuote: [
        () => getHookOrQuote(!!isGuest)
      ]
