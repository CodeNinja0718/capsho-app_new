...
export function downloadPDFTwitterCaptions(isPro: boolean) {
  const data = getTwitterCaptionsSource()
  const normalCaptions = normalizePdfTwitterCaptions(data.source, isPro)
  convertHtmlToPdf(normalCaptions, 'twitter-captions.pdf')
}

export function downloadDocxTwitterCaptions(isPro: boolean) {
  const data = getTwitterCaptionsSource()
...
export function normalizePdfTwitterCaptions(data: INormalCaption, isPro = false) {
  const heading = '<h4>Twitter Captions</h4>'
  let captionsBody = `
