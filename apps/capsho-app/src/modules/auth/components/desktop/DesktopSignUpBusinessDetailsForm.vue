  ...
  >
    <div class="column items-center q-gutter-y-lg full-width">
      <div class="row no-wrap justify-center full-width q-col-gutter-x-sm">
        ...
        <div
          v-if="false"
          class="col-6 text-center q-gutter-y-xs">
          <label
            class="inter-regular text-grey-7 text-subtitle1 text-weight-medium"
            for="podcastName">Your podcast name</label>
          <q-input
            id="podcastName"
            v-model="authStore.podcastName"
            color="accent"
            dense
            lazy-rules
            outlined
            placeholder="Please enter the name of your podcast"
            :rules="[ val => val && val.length > 0 || '*Podcast name is required']"
            type="text"
          />
        </div>
      </div>
