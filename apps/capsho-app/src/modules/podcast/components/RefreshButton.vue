<template>
  <q-btn
    class="text-weight-bold refresh-button"
    :color="color"
    flat
    :icon="(hovering && hoverIcon !== undefined ? hoverIcon : $attrs.icon)"
    size="md"
    v-bind="$attrs"
...
const color = computed(() => hovering.value ? 'orange' : 'readable-text-secondary')
