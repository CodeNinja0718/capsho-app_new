...
export function downloadPDFLinkedInCaptions(isPro: boolean, isGuestDrafts = false) {
  const data = isGuestDrafts ? getGuestLinkedInCaptionsSource() : getLinkedInCaptionsSource()
  const normalCaptions = normalizePdfLinkedInCaptions(data.source, isPro)
  convertHtmlToPdf(normalCaptions, 'linkedin-captions.pdf')
}

export function getPDFLinkedInCaptions(isPro: boolean, isGuestDrafts = false) {
  const data = isGuestDrafts ? getGuestLinkedInCaptionsSource() : getLinkedInCaptionsSource()
  const normalCaptions = normalizePdfLinkedInCaptions(data.source, isPro)
  return normalCaptions
}

export function downloadDocxLinkedInCaptions(isPro: boolean, isGuestDrafts = false) {
  const data = isGuestDrafts ? getGuestLinkedInCaptionsSource() : getLinkedInCaptionsSource()
...
export function normalizePdfLinkedInCaptions(data: INormalCaption, isPro = false) {
  const heading = '<h4 id="linkedin_caption">LinkedIn Captions</h4>'
  let captionsBody = `
...
export function getGuestLinkedInCaptionsSource() {
  const store = useUploadPodcastStore()
  const linkedInCaption = computed(() => {
    if (store.selectedEpisode.allCaptionsGuest) {
      if (store.selectedEpisode.allCaptionsGuest.linkedin) {
        return store.selectedEpisode.allCaptionsGuest.linkedin
      }
    }
    return store.allCaptionsGuest.linkedin
  })
  const normalData = normalizeCaptions(linkedInCaption.value)
  return {
    source: normalData
