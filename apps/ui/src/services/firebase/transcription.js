 ...
 */
export async function listenForGuestTranscriptUpdates (podcastId) {
  const { triggerNegative } = useNotification()
  const auth = getAuth()
  try {
    if (!auth.currentUser) {
    ...
    const unsubscribe = onSnapshot(doc(db, 'transcripts', docId), (doc) => {
      if (doc.exists()) {
        ...
        unsubscribe()
        Promise.resolve(store.createGuestInitialPromoDrafts())
      }
    })
  } catch (err) {
    if (err instanceof Error) {
 ...
 * @returns {Promise<{unsub: Unsubscribe, data}>}
 */
export async function listenForHostTranscriptUpdates (podcastId) {
  const { triggerNegative } = useNotification()
  const auth = getAuth()
  try {
    if (!auth.currentUser) {
    ...
    const unsubscribe = onSnapshot(doc(db, 'transcripts', docId), (doc) => {
      if (doc.exists()) {
        uploadPodcastStore.transcript = {
          ...doc.data()
        }
        alert('Topic is ready to choose')
        unsubscribe()
      }
    })
  } catch (err) {
