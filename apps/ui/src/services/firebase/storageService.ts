...
import { AudioFile, ImageFile, UploadableImage } from 'src/models/uploadableFile'
...
export async function saveImageForTemplate(imgData: ImageFile) {
  console.log('---1---')
  const auth = getAuth()
  const user = auth.currentUser
  if (!user || !imgData.file) {
  ...
  console.log('---2---')
  const storage = getStorage()
  const storageRef = ref(storage, 'template-images/' + generateUUID())
  const { triggerWarning } = useNotification()
  console.log('---3---')
  try {
    await uploadBytes(storageRef, imgData.file)
    ...
    if (err instanceof Error) {
      logtail?.error('storageService.ts - SaveImageForTemplage', {
        message: err.message
      })
      Promise.resolve(slackFrontendErrors({
        method: 'POST',
          ...
          blocks: [
            {
              ...
              text: {
                type: 'plain_text',
                text: ':ghost: saveImageForTemplage',
                emoji: true
              }
            },
            {
...
export async function saveImageBlock(imgData: UploadableImage) {
  const auth = getAuth()
  const user = auth.currentUser
  if (!user || !imgData.file) {
    return null
  ...
  const storage = getStorage()
  const storageRef = ref(storage, 'imgBlock/' + user.uid)
  const { triggerWarning } = useNotification()
  try {
    await uploadBytes(storageRef, imgData.file)
    return await getDownloadURL(storageRef)
