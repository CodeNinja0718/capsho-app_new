import { createSharedTemplate } from 'src/services/firebase/usersData'
...
export async function createUserWithEmail (
  ...
  const auth = getAuth()
  return lookUpDomain(String(email))
    .then((isDomainBanned: boolean) => {
      if (isDomainBanned) {
        return null
      }
...
export function verifyEmail () {
  ...
  const actionCodeSettings = {
    url: window.location.origin + '/auth/sign-in?' + getUserType()
  }
  if (auth.currentUser) {
