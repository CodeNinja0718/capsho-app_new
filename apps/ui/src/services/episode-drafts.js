...
export async function generateBlogDraftsOne (data) {
  const { blogType, blogSeoQuestion, transcriptId, podcastId } = data
  const response = await cloudFunction('blogDraftsApi-generateBlogPostLADrafts')({
    blogType,
    blogSeoQuestion,
    transcriptId,
    podcastId
...
export async function generateBlogDraftsTwo (data) {
  const { transcriptId, podcastId, blogTopic, blogType } = data
 ...
 * @returns {Promise<*>}
 */
export async function generateBlogTopics (data) {
  const { blogType, transcriptId, podcastId } = data
  const response = await cloudFunction('blogDraftsApi-generateBlogTopics')({
    blogType,
    transcriptId,
    podcastId
  })
