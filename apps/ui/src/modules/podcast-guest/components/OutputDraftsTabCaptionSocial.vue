...
import AppSkeleton from 'src/components/base/AppSkeleton.vue'
...
const captionTemplate = computed<string>({
  get() {
    ...
    if (store.allCaptionsGuest && store.allCaptionsGuest.facebook && Object.keys(store.allCaptionsGuest.facebook).length) {
      return store.allCaptionsGuest.facebook[key]
    }
    return ''
  },
  set(value: string) {
    const key = captionType.value.value
    store.allCaptionsGuest.facebook[key] = htmlSanitizer(value)
  }
})
...
const updateCaptions = debounce(() => {
  ...
  if (captionTemplate.value) {
    if (store.selectedEpisode.allCaptionsGuest) {
      store.selectedEpisode.allCaptionsGuest.facebook[key] = captionTemplate.value
    }
