...
const captionTemplate = computed<string>({
  get() {
    ...
    if (store.allCaptionsGuest && store.allCaptionsGuest.linkedin && Object.keys(store.allCaptionsGuest.linkedin).length) {
      return store.allCaptionsGuest.linkedin[key]
    }
    return ''
  },
  set(value: string) {
    const key = captionType.value.value
    store.allCaptionsGuest.linkedin[key] = htmlSanitizer(value)
  }
})
...
const updateCaptions = debounce(() => {
  ...
  if (captionTemplate.value) {
    if (store.selectedEpisode.allCaptionsGuest) {
      store.selectedEpisode.allCaptionsGuest.linkedin[key] = captionTemplate.value
    }
...
const sampleCaption = 'Are you ready to use The Agents Growth Academy to get exponential growth, increased profitability, and improved relationships? This week\'s episode has all the answers you need. Listen now and see how you can benefit from the Academy.'
</script>
    ...
    <div class="row justify-between q-pb-xs">
      <div class="col-11">
        <div>
          ...
          >
            <span>
              LinkedIn Caption is generating - Please wait
            </span>
            <q-spinner-dots
              color="grey-7"
              size="1.2em"
