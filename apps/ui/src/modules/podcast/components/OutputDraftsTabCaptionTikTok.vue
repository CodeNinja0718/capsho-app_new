...
const captionTemplate = computed<string>({
  get() {
    ...
    if (store.allCaptions && store.allCaptions.tiktok && Object.keys(store.allCaptions.tiktok).length) {
      return store.allCaptions.tiktok[key].replace(pattern, 'border: 1px solid #9e9e9e;')
    }
    return ''
  },
  set(value: string) {
    const key = captionType.value.value
    store.allCaptions.tiktok[key] = htmlSanitizer(value)
  }
})
...
const updateCaptions = debounce(() => {
  ...
  if (captionTemplate.value) {
    store.selectedEpisode.allCaptions.tiktok[key] = captionTemplate.value
