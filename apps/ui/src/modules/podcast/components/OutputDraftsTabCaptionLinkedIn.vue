...
const captionTemplate = computed<string>({
  get() {
    ...
    if (store.allCaptions && store.allCaptions.linkedin && Object.keys(store.allCaptions.linkedin).length) {
      return store.allCaptions.linkedin[key].replace(pattern, 'border: 1px solid #9e9e9e;')
    }
    return ''
  },
  set(value: string) {
    const key = captionType.value.value
    store.allCaptions.linkedin[key] = htmlSanitizer(value)
  }
})
...
const updateCaptions = debounce(() => {
  ...
  if (captionTemplate.value) {
    store.selectedEpisode.allCaptions.linkedin[key] = captionTemplate.value
