...
watchEffect(() => {
  if (Object.keys(store.transcript).length) {
    if (store.transcript.editedTranscript?.length) {
      transcript.text = replaceTranscriptLabel(store.transcript.editedTranscript)
    } else if (store.transcript.labeledText?.length) {
      transcript.text = replaceTranscriptLabel(store.transcript.labeledText)
    } else {
      transcript.text = replaceTranscriptLabel(store.transcript.text)
    }
  }
})

function replaceTranscriptLabel(text: string) {
  if (!text) {
    return ''
  }
  const speakerLabelPattern = /rgba\(0, 0, 0, 0.85\)/gm
  const labelPatternReplacer = '#525252'
  const timeLabelPattern = /rgba\(0, 0, 0, 0.55\)/gm
  return text
    .replace(speakerLabelPattern, labelPatternReplacer)
    .replace(timeLabelPattern, labelPatternReplacer)
}
      ...
      <div class="col-11">
        ...
        <div class="full-width row no-wrap">
          ...
          <q-btn
            ...
            <template #default>
              <div class="text-grey-3">Restore</div>
            </template>
          </q-btn>
        </div>
      </div>
