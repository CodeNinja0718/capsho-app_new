...
const captionTemplate = computed<string>({
  get() {
    ...
    if (store.allCaptions && store.allCaptions.twitter && Object.keys(store.allCaptions.twitter).length) {
      return store.allCaptions.twitter[key].replace(pattern, 'border: 1px solid #9e9e9e;')
    }
    return ''
  },
  set(value: string) {
    const key = captionType.value.value
    store.allCaptions.twitter[key] = htmlSanitizer(value)
  }
})
...
const updateCaptions = debounce(() => {
  ...
  if (captionTemplate.value) {
    store.selectedEpisode.allCaptions.twitter[key] = captionTemplate.value
